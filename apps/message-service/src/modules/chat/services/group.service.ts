...
export class GroupService {
  ...
  async createGroupChatFunction(
    ...
    const blockedUsers = await getBlockedRelationshipUserIds(
      userIds,
      currentUserId,
    );

    const users = new Set(blockedUsers.userIds);

    const allowedUserIds = userIds.filter((id) => !users.has(id));
    ...
    const memberUsers = await fetchUsers(members.map(String));
  ...
  async getGroupByIdFunction(userId: string, chatId: string) {
    ...
    const members = await fetchUsers(group.members.map(String));
  ...
  async addMembersFunction(chatId: string, members: string[], userId: string) {
    ...
    const blockedUsers = await getBlockedRelationshipUserIds(members, userId);

    const blockedUserIds = new Set(blockedUsers.userIds);

    const allowedUserIds = members.filter((id) => !blockedUserIds.has(id));
    ...
    const memberUsers = await fetchUsers(updatedGroup.members.map(String));
